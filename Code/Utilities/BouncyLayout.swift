...
public class BouncyLayout: UICollectionViewFlowLayout {
    ...
    public override func prepareLayout() {
        super.prepareLayout()
        guard let view = collectionView, let attributes = super.layoutAttributesForElementsInRect(view.bounds)?.flatMap({ $0.copy() as? UICollectionViewLayoutAttributes }) else { return }
    ...
    public override func shouldInvalidateLayoutForBoundsChange(newBounds: CGRect) -> Bool {
        ...
        animator.behaviors.forEach {
            ...
            update(behavior, and: item, in: view, for: newBounds)
    ...
    private func update(behavior: UIAttachmentBehavior, and item: UIDynamicItem, in view: UICollectionView, for bounds: CGRect) {
        ...
        let resistance = CGVector(dx: fabs(view.panGestureRecognizer.locationInView(view).x - behavior.anchorPoint.x) / 1000, dy: fabs(view.panGestureRecognizer.locationInView(view).y - behavior.anchorPoint.y) / 1000)
